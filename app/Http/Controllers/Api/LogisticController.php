...
class LogisticController extends Controller
{
    ...
    public function service_transport_row(){
        ...
        ->take(100)->select('transports.created_at as created_at','transports.id as id','organizations.name as organization','services.name as service','transports.service_count as service_count',
        'transports.accrued as accrued', 'transports.salary_id as salary_id','users.name as user')->orderBy('transports.id','DESC')->get();
        return $transports;
    }
    ...
    public function transport_entries(){
        ...
        foreach($transport as $tr){
            $service_id=0;
            if($tr->service_id==10){ $service_id = 19; }
            if($tr->service_id==12){ $service_id = 20; }
            if($tr->service_id==13){ $service_id = 21; }
            if($tr->service_id==15){ $service_id = 18; }
            if($tr->service_id==16){ $service_id = 17; }
            if($tr->service_id==14){ if($tr->service_count>3){$service_id = 5;}else{$service_id = 4;} }
            if($service_id>0){
