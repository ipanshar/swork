...
public function myWork(Request $request){
   ...
   $operations=Operation::orderByDesc('update_at')->where('user_id',$user->id)->where('created_at','>',$request->GetCreated)->where('created_at','<',$request->EndCreated)->groupBy('application_id')->select('application_id',DB::raw('sum(num) as num_me,date(max(updated_at)) update_at'))->get();
   ...
   foreach ($operations as $oper){
      ...
      $box=Box::where('application_id',$oper->application_id)->select(DB::raw('count(id) as num_me, avg(rate) as rate'))->first();
      if ($box->num_me>0){
